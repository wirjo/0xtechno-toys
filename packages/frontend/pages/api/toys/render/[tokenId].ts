...
export default async function handler(req: Request, res: Response): Promise<any> {
  ...
  if (checkIfRenderedOnCdn) return res.redirect(checkIfRenderedOnCdn as string);
  else
    return res.redirect(
      `https://${renderedStorageBucket}.us-east-1.amazonaws.com/${tokenIdNumber}.png`,
    );
}
...
async function tokenRendered(tokenIdNumber: number) {
  ...
  try {
    ...
    if (nftImageStatus == 200) return renderedUrl;
    else return false;
  } catch (e) {
    console.error(e);
  }
